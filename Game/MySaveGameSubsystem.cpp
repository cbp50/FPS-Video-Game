...
void UMySaveGameSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);

	const UMySaveGameSettings* SGSettings = GetDefault<UMySaveGameSettings>();
...
void UMySaveGameSubsystem::HandleStartingNewPlayer(AController* NewPlayer)
{
...
bool UMySaveGameSubsystem::OverrideSpawnTransform(AController* NewPlayer)
{
...
void UMySaveGameSubsystem::SetSlotName(FString NewSlotName)
{
	// Ignore empty name
...
void UMySaveGameSubsystem::WriteSaveGame()
{
	...
	{
		AMyPlayerState* PS = Cast<AMyPlayerState>(GS->PlayerArray[i]);
		if (PS)
		{
	...
	for (FActorIterator It(GetWorld()); It; ++It)
	{
		...
		if (Actor->IsPendingKill() || !Actor->Implements<UMyInterface>())
		{
			continue;
		}
...
void UMySaveGameSubsystem::LoadSaveGame(FString InSlotName /*= ""*/)
{
